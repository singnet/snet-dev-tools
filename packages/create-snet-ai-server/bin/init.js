...
async function promptQuestions() {
  const questions = [
    ...
    },
    {
      ...
      validate: (value) => (Boolean(value.trim()) ? true : "Please enter the organization id"),
    },
    {
      type: "text",
      name: "serviceId",
      message: "Service ID of the service",
      validate: (value) => (Boolean(value.trim()) ? true : "Please enter the service id"),
    },
    {
      type: "confirm",
      name: "askConfig",
      message: "Do you like to enter the configs here, so that we can prefill them for you in their right places",
      initial: true,
    },
  ];
  ...
  if (response.askConfig) {
    const configPromptQuestions = [
      ...
      },
      {
        type: "text",
        name: "freeCallToken",
        message: "Freecall Token (You can download it in beta.singularitynet.io)",
      },
      {
        type: "text",
...
module.exports = async function () {
  ...
  progress.stop();
  console.log(chalk.green("Success ✅ :"), chalk.green("Your snet server has been scaffolded successfully."));
  console.log(
    chalk.white(
  ...
  console.log(chalk.green("1:"), chalk.white("Update the import paths for service and message grpc stubs"));
  console.log(chalk.green("2:"), chalk.white("Initialize the request object and the set the required input values"));
  console.log(chalk.green("3:"), chalk.white("Change the method name according to your service"));
  console.log(chalk.green("4:"), chalk.white("Parse the proto get the output in the desired format"));
};
